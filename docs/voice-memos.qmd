...
To make the credential part of the Voice Memos job definition while keeping
its value outside YAML, configure an environment variable or mounted file:
...
Use `file: /var/run/secrets/hugging-face/token` instead in a container or
Kubernetes workload with a mounted Secret. This credential is used only by
`diarize` and `diarize-parallel`.
